...
- **Booking status vs. confirmation state** — `status` (Pending / In Progress / Completed / Cancelled / No-show) is a real column stored in the database. "Confirmation state" (Unconfirmed / Confirmed / etc., shown as a colored badge) is _computed_ from `status` plus `payment_status`, `booking_source`, and `service_category` — it's never stored, so it can't drift out of sync with the real payment record.
