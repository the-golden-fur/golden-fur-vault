...
>
> **Update (2026-09-02):** paths and names below are superseded by the
> `sessions/` restructure. Reviews now land at
> `Projects/golden-fur/sessions/reviews/<branch>/`; the verification-record
> agent `testing-documenter` is now `session-documenter` (writes the whole
> `sessions/NN-<slug>/` set); the personal `settings.local.json` backstop is
> now a checked-in `pr-guard` hook. The reasoning below still holds.
