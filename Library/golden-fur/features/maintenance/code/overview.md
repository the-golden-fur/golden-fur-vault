...
- **`services.service.ts`** — CRUD for `services`. `createService`/`updateService` mirror `first_hour_fee` into `base_price` for Daycare (since Daycare's real price is computed at checkout, not admin-entered). `attachPricingMatrix` derives the read-time Grooming size/coat grid via `deriveGroomingMatrix` and attaches it under the same `service_pricing_tiers` key old consumers expect, with synthesized (non-DB) ids. `setServiceBranchAvailability` is the _only_ place `services.is_active` changes now — it's kept in sync as "active whenever at least one branch has it available."
...
- **`petWeightClassConfiguration.service.ts`** — get/update for the singleton S/M/L/XL kg cut-off row. `updatePetWeightClassConfiguration` re-checks `m < l < xl` against the _merged_ stored+patch values (the Postgres CHECK constraint is the final backstop).
